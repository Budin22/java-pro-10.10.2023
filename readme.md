...
- Описати цю сутність за допомогою JPA-анотацій.
...
-  Додаток повинен бути доступний за URL: http://localhost:8080
-  Налаштувати
-  Налаштувати логування на рівні INFO для пакетів програми та для пакету org.springframework.web
...
- Реалізувати метод отримання за id
- Реалізувати метод отримання все
- Реалізувати метод додавання
- Реалізувати метод видалення
3. Реалізувати клас User. Цей клас зберігатиме значення: id, name, password, role
4. Реалізувати клас SecurityConfiguration.
- Цей клас повинен забороняти доступ до програми для неавторизованих користувачів
- Цей клас повинен відкрити можливість автентифікації за допомогою Basic Authentication. Вхідні дані порівнюються з даними в БД
- Операції додавання, видалення замовлення повинні бути доступні тільки для користувача за участю Admin
5. Налаштувати Spring-додаток через application.yml
- Додаток має бути доступний за URL: http://localhost:8080
- Налаштувати підключення в БД
- Налаштувати логування на рівні INFO для пакетів програми та пакету org.springframework.web
...
- Цей контролер доступний всім користувачам
8. Реалізувати контролер для взаємодії з ресурсом Product.
- Контролер доступний за URL: http://localhost:8080/products
...
- логування на рівні INFO для пакетів програми та для пакету org.springframework.web
Процес логування включає виведення в консоль та запис у файл
